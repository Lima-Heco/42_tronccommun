...
/*   Updated: 2023/10/25 11:24:12 by lheinric         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../fd_printf.h"

static int	print_nbrbase(int fd, int len, char *digits, unsigned long long n)
{
	char	*buffer;
	int		i;
...
static int	putpt_base(int fd, unsigned long long n, char *base)
{
	...
	temp = n;
	return (print_nbrbase(fd, len, base, temp));
}

int	ft_pttostr(int fd, unsigned long long n)
{
	if (n == 0)
